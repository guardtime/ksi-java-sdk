 ...
 */
public class UserProvidedPublicationCalendarHashChainAlgorithmDeprecatedRule extends BaseRule {

    private static final Logger logger =
            LoggerFactory.getLogger(UserProvidedPublicationCalendarHashChainAlgorithmDeprecatedRule.class);

    public VerificationResultCode verifySignature(VerificationContext context) throws KSIException {
        Date publicationTime = context.getUserProvidedPublication().getPublicationTime();
        CalendarHashChain extendedCalendarHashChain = context.getExtendedCalendarHashChain(publicationTime);
        if (isAlgorithmsDeprecated(extendedCalendarHashChain, publicationTime)) {
